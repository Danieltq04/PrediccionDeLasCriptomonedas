...
io.on("connection", (socket) => {
  ...
  socket.on("conectado", (nomb) => {
    i++;
    io.emit("u_conectados",i);
    nombre = nomb;

    console.log("Usuarios Conectados: ",i);
  ...
  socket.on("criptomoneda", (moneda) => {
    contador++;
    console.log("Entro ", contador, " veces");
...
function mostrarValores(imagen, moneda, precio_actual, precio_hace_1_dia, porcentaje_7_dias, prediccion_1, prediccion_7, parametros, valores, obs) {
  ...
  if(moneda == "bitcoin"){
    obs = observandoresBitcoin;
  }
  else if(moneda == "ethereum"){
    obs = observandoresEthereum;
  }
  else if(moneda == "tether"){
    obs = observandoresTether;
  }
  else if(moneda == "dogecoin"){
    obs = observandoresDogecoin;
  }
  io.emit("valores_estadisticos"+moneda, {
    imag: imagen,
    c_moneda: moneda.toUpperCase(),
    actual: precio_actual.toFixed(2),
    p_1: p_1_dia,
    p_7: p_7_dias,
    pred_1: ((prediccion_1+precio_actual*porcentaje_1_dia/100*2)/3).toFixed(2),
    pred_7: ((prediccion_7+precio_actual*porcentaje_7_dias/100*2)/3).toFixed(2),
